...
import { AccountModel, AuthenticationModel, LoadAccontByEmailRepository, HashComparer, Encrypter, UpdateAccessTokenRepository } from './db-authentication-protocols';
...
const makeSut = (): SutTypes => {
  ...
  const sut = new DbAuthentication(loadAccontByEmailRepositoryStub, hashComparerStub, encrypterStub, updateAccessTokenRepositoryStub);
  return {
    sut,
...
describe('Dbauthentication UseCase', () => {
  ...
  test('Should throw if LoadAccontByEmailRepository throws', async () => {
    const { sut, loadAccontByEmailRepositoryStub } = makeSut();
    jest.spyOn(loadAccontByEmailRepositoryStub, 'loadByEmail').mockReturnValueOnce(new Promise((_resolve, reject) => reject(new Error())));
  ...
  test('Should return LoadAccontByEmailRepository returns null', async () => {
    const { sut, loadAccontByEmailRepositoryStub } = makeSut();
    jest.spyOn(loadAccontByEmailRepositoryStub, 'loadByEmail').mockReturnValueOnce(null);
